		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			try
			{
				...
				for(int nIndexer = 0;nIndexer < nNumberOfLabels;nIndexer++)
				{
                    ...
                    myGraphics.DrawString(strSplitLabels[nIndexer], fntLabelFont, brSolidBrush, fDividerCounter - ((fntLabelFont.SizeInPoints) * strSplitLabels[nIndexer].Length)/6, fLeftRow);
				}
				...
				if (
                    (
                    strNewRange1 != null && strNewRange2 != null && 
					(!strRange1.Equals(strNewRange1) || !strRange2.Equals(strNewRange2)) ||
					(!bMouseEventThumb1 && !bMouseEventThumb2))
                    )
				{
                    //brSolidBrush		= new System.Drawing.SolidBrush(this.BackColor);
                    //strRangeOutput = strRange1 + " - " + strRange2 + strRangeString.Length + "A";
                    //strRangeOutput = strRangeString.Length + "A";
                    ...
                    if (!String.IsNullOrEmpty(strRangeString) && strRangeString.Trim() != "")
                    {
                        brSolidBrush = new System.Drawing.SolidBrush(clrStringOutputFontColor);
